...
export function initAudio() { if (AC) return; AC = new (window.AudioContext || window.webkitAudioContext)(); master = AC.createGain(); master.gain.value = settings.vol; master.connect(AC.destination); }
export function resumeAudio() { if (AC && AC.state === 'suspended') AC.resume(); }
export function setVolume(v) { if (master) master.gain.value = v; }
function noiseBuf(brown) { const len = AC.sampleRate * 3, b = AC.createBuffer(1, len, AC.sampleRate), d = b.getChannelData(0); let last = 0; for (let i = 0; i < len; i++) { const w = Math.random() * 2 - 1; if (brown) { last = (last + 0.02 * w) / 1.02; d[i] = last * 3.5; } else d[i] = w; } return b; }
...
export function startAmbience(type, timeKey) {
  ...
  src.start(); lfo.start(); ambNodes.push(src, lfo, f, g, lg);
  if (type === 'forest') {
...
function pop() { const t = AC.currentTime, s = AC.createBufferSource(); s.buffer = noiseBuf(false); const f = AC.createBiquadFilter(); f.type = 'bandpass'; f.frequency.value = rnd(1200, 3200); f.Q.value = 1.5; const g = AC.createGain(); g.gain.setValueAtTime(rnd(0.015, 0.05), t); g.gain.exponentialRampToValueAtTime(0.0001, t + 0.05); s.connect(f); f.connect(g); g.connect(master); s.start(t); s.stop(t + 0.08); }
...
export function sfx(type) {
  ...
  s.connect(f); f.connect(g); g.connect(master); s.start(t); s.stop(t + 0.5);
}
