...
/* RenderPass → GTAO → Bokeh(사진) → Bloom → OutputPass → Grade
   Grade: 채도·대비·틴트 + 스플릿 토닝(그림자는 하늘 색, 하이라이트는 햇빛 색) + 완만한 필름 커브 + 비네트 + 그레인 */
const GradeShader = {
  uniforms: { tDiffuse: { value: null }, uTime: { value: 0 }, vig: { value: 0.35 }, grain: { value: 0.04 }, sat: { value: 1 }, con: { value: 1 }, tint: { value: new THREE.Vector3(1, 1, 1) }, sTint: { value: new THREE.Vector3(1, 1, 1) }, hTint: { value: new THREE.Vector3(1, 1, 1) }, filmK: { value: 0.3 } },
...
/* 색의 밝기를 1 로 정규화한 "색조"를 흰색에 k 만큼 섞은 틴트 */
...
export function createPost(renderer, camera) {
  /* 컴포저 타깃: HalfFloat(HDR) + MSAA. renderer 의 antialias 는 컴포저를 쓰면 무효라 여기서 켜야 한다 */
